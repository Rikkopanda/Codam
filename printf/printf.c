...
{
	ft_strcpy(tagrules->LEN_h_SPECIFIERS, "idouxX");
	ft_strcpy(tagrules->LEN_I_SPECIFIERS, "idouxX");
	ft_strcpy(tagrules->LEN_I_SPECIFIERS, "eEfgG");
...
void f_join_str(char specifier, char *print_str, va_list args, char *ret_chars)
{
	char *pre_str;
	char *new_str;
	pre_str = ft_calloc(sizeof(char) , 12);
	//printf("chars = %s ", ret_chars);
	if(ft_strchr(ret_chars, '#'))
	{
		char str[] = "0X";
		if(specifier == 'o')
			ft_strcpy(pre_str, "0");
		...
		new_str = ft_strjoin(pre_str, print_str);
		//printf("\n str %s\n" , print_str);
...
int ft_format(const char *format, char *print_str, va_list args, int *i)
{
	...
	if(print_str == NULL)
			return(1);
	if(ft_strlen(ret_chars) > 0)
		f_join_str(format[*i], print_str, tagsrules.pre_str, ret_chars);
	else
